...
// new entry via `onAdd`.
export default function ActivityTimeline({ activities, onAdd, onVoid }) {
  ...
  return (
    <div>
      ...
      {activities && activities.map((a) => {
        ...
        return (
          <div key={a.id} style={{ display: "flex", gap: 10, marginBottom: 14, opacity: voided ? 0.45 : 1 }}>
            ...
            <div style={{ paddingBottom: 4, flex: 1 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "start" }}>
                <div style={{ fontSize: 13.5, fontWeight: 700 }}>
                  {meta.label}{voided && <span style={{ fontWeight: 700, color: "var(--bad)", fontSize: 11, marginLeft: 6 }}>VOIDED</span>}
                </div>
                {onVoid && (
                  <span style={{ fontSize: 11, color: "var(--mut)", cursor: "pointer", textDecoration: "underline", flexShrink: 0 }} onClick={() => onVoid(a)}>
                    {voided ? "Restore" : "Void"}
                  </span>
                )}
              </div>
              {a.note && <div style={{ fontSize: 13, color: "var(--mut)", marginTop: 2, textDecoration: voided ? "line-through" : "none" }}>{a.note}</div>}
              <div style={{ fontSize: 11.5, color: "var(--mut)", marginTop: 3 }}>
                {timeAgo(a.createdAt)}{a.createdBy ? ` · ${a.createdBy}` : ""}
                {voided && a.voidedBy ? ` · voided by ${a.voidedBy}` : ""}
              </div>
            </div>
          </div>
        );
